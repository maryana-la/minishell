...
{
	if (!ft_strncmp(all->cmnd.args[0], "pwd", 4))
		pwd_command(all);
	else if (!ft_strncmp(all->cmnd.args[0], "env", 4))
		print_env_list(all->env_vars, 0, all->env_counter);
	else if (!ft_strncmp(all->cmnd.args[0], "export", 7))
		export_command(all);
	else if (!ft_strncmp(all->cmnd.args[0], "unset", 6))
		unset_command(all);
	else if (!ft_strncmp(all->cmnd.args[0], "cd", 3))
		cd_command(all);
	else if (!ft_strncmp(all->cmnd.args[0], "echo", 5))
		echo_command(all);
	else if (!ft_strncmp(all->cmnd.args[0], "exit", 5))
		exit_command(all);
	else
		cmd_exec(all);
}
...
//todo -n в середине текста отображается, но срабатывает как флаг
	...
	{
		if (!ft_strncmp (all->cmnd.args[i], "-n", 2) && !flag)
		{
			j=0;
			while (all->cmnd.args[i][++j] == 'n');
			if (j != ft_strlen(all->cmnd.args[i]))
			{
				flag = 1;
				if (all->cmnd.args[i+1])
					printf("%s ", all->cmnd.args[i]);
				else
					printf("%s", all->cmnd.args[i]);
				continue;
			}
			if (ft_strncmp (all->cmnd.args[i + 1], "-n", 2))
			{
				flag = 1;
				continue;
			}
...
void export_command(t_all *all) {
	...
	while (all->cmnd.args[++all->arg_pos])
	{
...
void add_new_variable(t_all *all)
{
	...
	if (ft_isdigit(all->cmnd.args[all->arg_pos][0]) || !ft_strcmp(all->cmnd.args[all->arg_pos], "="))
	{
		error_handler(all, 1);
		return;
	}

	while (all->cmnd.args[all->arg_pos][++j])
	{
		if (all->cmnd.args[all->arg_pos][j] == '=')
		{
			ravno = 1;
			break;
		}
	}
	temp_key = ft_substr(all->cmnd.args[all->arg_pos], 0, j);
	temp_value = ft_substr(all->cmnd.args[all->arg_pos], j + 1, ft_strlen(all->cmnd.args[1])-j+1);
//	if (temp_value[0] == '\0' && ravno)
	...
	tmp[i].key = ft_substr(all->cmnd.args[all->arg_pos], 0, j);
	tmp[i].value = ft_substr(all->cmnd.args[all->arg_pos], j + 1, ft_strlen(all->cmnd.args[1])-j+1);
...
void unset_command(t_all *all)
{
	...
	while (all->cmnd.args[++j])
	{
		i = -1;

		while(++i < all->env_counter && ft_strcmp(all->env_vars[i].key, all->cmnd.args[j]));
		if (i != all->env_counter)
		{
