...
if [ "$1" == "my" ] || [ "$1" == "all" ]; then
    ...
    exec_test 'env puis export puis env'
    exec_test 'cat Makefile | grep pr | head -n 5 | cd test (mybin)'
    exec_test 'cat Makefile | grep pr | head -n 5 | cat test (bin)'
    exec_test 'cat Makefile | grep pr | head -n 5 | hello (NA)'
    ...
    exec_test 'cat < test'
    exec_test 'echo bonjour > $test'
    exec_test 'file_name_in_current_dir'
